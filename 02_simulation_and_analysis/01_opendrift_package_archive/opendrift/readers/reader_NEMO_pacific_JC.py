# copied from ROMS reader and edited to work for NEMO dataset
...
class Reader(BaseReader):
    ...
    def __init__(self, filename=None, name=None, gridfile=None):
        ...
        #JC: comment out part of else so that variable reference doesn't change
        else:
            self.num_layers = 1
        #    self.ROMS_variable_mapping['ubar'] = 'x_sea_water_velocity'
        #    self.ROMS_variable_mapping['vbar'] = 'y_sea_water_velocity'
        #    del self.ROMS_variable_mapping['uo']
        #    del self.ROMS_variable_mapping['vo']

        # JC: change variables that get referred to
        if 'nav_lat' in self.Dataset.variables:
            ...
            if gridfile is None:
            # JC: I don't think I am supposed to have a spearate grid file.
        ...
        try:
            ...
            ocean_time = self.Dataset.variables['time_counter'] #JC: match time variable name
        if has_xarray:
        ...
        if has_xarray:
            ...
            # JC: this is key. This will reference the variable names for the grid that my data is on.
            self.xmax = np.float(len(self.Dataset.dimensions['x'])) - 1
            self.ymax = np.float(len(self.Dataset.dimensions['y'])) - 1

        self.name = 'nemo pacific jc custom' #JC
    ...
    def get_variables(self, requested_variables, time=None,
        ...
        for par in requested_variables:
            ...
            if par == 'land_binary_mask':
                # JC: commment out. Don't have mask_rho. I think Opendrift will default to using the reader_basemap_landmask as the mask.
            ...
            if par not in mask_values:
                if has_xarray is False:
                    ...
                    indxgrid = indx
                    indygrid = indy
                # JC: I don't have mask_rho values
            ...
            #JC : this was already commented out
        ...
        # JC: I don't think rotation is applicable for my NEMO data. Comment out below.
